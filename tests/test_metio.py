...
class TestGridMet(unittest.TestCase):
    ...
    def test_conforming_array(self):
        ...
        bounds = RasterBounds(affine_transform=l5.affine, profile=l5.profile)
    ...
    def test_save_array(self):
        ...
        tif_dir = os.path.join(base, 'data', 'image_test', 'lt5_image')
        l5 = Landsat5(tif_dir)
        polygon = l5.get_tile_geometry()
        ...
        gridmet.get_data_subset(grid_conform=True)
        gridmet.save(gridmet, geometry=l5.rasterio_geometry, output_filename=os.path.join(tif_dir, 'gridmet.tif'))
        # self.assertEqual(gridmet.pr.shape, self.image_shape)
